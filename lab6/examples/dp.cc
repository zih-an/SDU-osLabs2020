...
dp::dp(int r) {
    ...
    //5 个中同时可以有 2 个在就餐，建⽴⼀个初值为 2 的⽤于锁的信号灯
    ...
    lock = new Lock(sema);  // 用共享的信号量创建锁（可以有2个在就餐）
    for (int i = 0; i < 5; i++) {
        //为每个哲学家建⽴⼀个条件变量和可共享的状态 //初始状态都为思考
        if ((state[i] = (char *)set_shm(shm_key++, shm_num, ipc_flg)) == NULL) {
            perror("Share memory create error");
            exit(EXIT_FAILURE);
        }
        *state[i] = thinking;
        //为每个哲学家建⽴初值为 0 的⽤于条件变量的信号灯
...
void dp::pickup(int i) {
    lock->close_lock();//进⼊管程，上锁
    *state[i] = hungry; //进饥饿态
    self[i]->Wait(lock, i); //测试是否能拿到两只筷⼦
...
int main(int argc, char *argv[]) {
    ...
    tdp = new dp(rate); //建⽴⼀个哲学家就餐的管程对象

    pid[0] = fork(); //建⽴第⼀个哲学家进程
    if (pid[0] < 0) {
        perror("p1 create error");
        exit(EXIT_FAILURE);
    } 
    else if (pid[0] == 0) { //利⽤管程模拟第⼀个哲学家就餐的过程
        while (1) {
            tdp->pickup(0);//拿起筷⼦
            tdp->putdown(0);//放下筷⼦
        }
    ...
    pid[1] = fork(); //建⽴第⼆个哲学家进程
    if (pid[1] < 0) {
        perror("p2 create error");
        exit(EXIT_FAILURE);
    } 
    else if (pid[1] == 0) { //利⽤管程模拟第⼆个哲学家就餐的过程
        while (1) {
            tdp->pickup(1);//拿起筷⼦
            tdp->putdown(1);//放下筷⼦
        }
    ...
    pid[2] = fork(); //建⽴第三个哲学家进程
    if (pid[2] < 0) {
        perror("p3 create error");
        exit(EXIT_FAILURE);
    } 
    else if (pid[2] == 0) { //利⽤管程模拟第三个哲学家就餐的过程
        while (1) {
            tdp->pickup(2);//拿起筷⼦
            tdp->putdown(2);//放下筷⼦
        }
    ...
    pid[3] = fork(); //建⽴第四个哲学家进程
    if (pid[3] < 0) {
        perror("p4 create error");
        exit(EXIT_FAILURE);
    } 
    else if (pid[3] == 0) { //利⽤管程模拟第四个哲学家就餐的过程
        while (1) {
            tdp->pickup(3);//拿起筷⼦
            tdp->putdown(3);//放下筷⼦
        }
    ...
    pid[4] = fork(); //建⽴第五个哲学家进程
    if (pid[4] < 0) {
        perror("p5 create error");
        exit(EXIT_FAILURE);
    } 
    else if (pid[4] == 0) { //利⽤管程模拟第五个哲学家就餐的过程
        while (1) {
            tdp->pickup(4);//拿起筷⼦
            tdp->putdown(4);//放下筷⼦
        }
